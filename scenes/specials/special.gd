...
signal left_camera_view()

var screen_size: Vector2i
var main_scene: Node = null
var initial_camera_x: float = 0.0  # Track camera position when flag was spawned
var initial_spawn_x: float = 0.0  # Track actual spawn x position (includes sprite half-width)
var has_entered_view: bool = false  # Track if we've already emitted the signal
var has_left_view: bool = false  # Track if we've already emitted the left view signal
...
func initialize(_speed: float, size: Vector2i, camera_x: float):
	...
	# Store the actual spawn x position (already set by SpecialEventManager with sprite half-width)
	initial_spawn_x = position.x
...
func _process(_delta: float) -> void:
	...
	# For parallax: object moves at 40% of camera movement
	...
	var flag_delta = camera_delta * 0.40  # Object moves 40% of camera movement
	# Use the actual spawn position (which includes sprite half-width from SpecialEventManager)
	position.x = initial_spawn_x + flag_delta
	...
	# Check if object has left camera view (completely off the left edge)
	if has_entered_view and not has_left_view:
		...
		# Object has left when its right edge is past the left edge of the camera view
		# This ensures the entire object is off-screen before we consider it "left view"
		# Camera position is center, so left edge of view is camera_x - screen_size.x / 2.0
		var camera_left_edge = camera_x - screen_size.x / 2.0
		if object_right_edge < camera_left_edge:
			has_left_view = true
			left_camera_view.emit()
